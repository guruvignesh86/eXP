...
import { useAuth } from "../Routings/Authentic";
 ...
 import AreaCart from "../Chart/AreaChart";
 import RadarCart from "../Chart/Radar";
...
useEffect(() => {
  const handleResize = () => {
    setIsSmallScreen(window.innerWidth < 768); // You can tweak this breakpoint
  };
          ...
          if (result.isConfirmed) {
            ...
            Swal.fire("Logged Out!", "You have been logged out.", "success").then(
              () => {
                ...
                window.location.replace("/F"); // Hard redirect to clear history
              }
            );
          }
        });
...
{isOpen && (
  <div className="fixed inset-0 bg-gray-800/30 backdrop-blur-md z-60 transition-opacity duration-300 opacity-60"></div>
)}
...
> */}
<div
  className={`flex-1 p-6 bg-white space-x-6 relative z-10 `}
  ref={sidebarRef}
>
...
">
  ...
  <div className="sm:hidden fixed top-0 -right-8 z-10 mt-5 ">
        <button
...
<div className="flex flex-wrap justify-center gap-6 mt-6 px-4">
  ...
  style={{ marginLeft: isSmallScreen ? "-180px" : "0px" }}
>
  {activeTab === "chart" && <ExpenseChartF expenses={sortedExpenses} />}
</div>
  <div
  className="w-full md:w-[48%] lg:w-[48%]"
  style={{ marginLeft: isSmallScreen ? "-120px" : "0px" }}
>
  {activeTab === "chart" && <AreaCart expenses={sortedExpenses} />}
</div>
  <div
  className="w-full md:w-[48%] lg:w-[48%]"
  style={{ marginLeft: isSmallScreen ? "-170px" : "0px" }}
>
  {activeTab === "chart" && <RadarCart expenses={sortedExpenses} />}
</div>
...
<div className="w-full  h-full overflow-x-auto lg:overflow-x-hidden overflow-y-hidden -ml-8 sm:-ml-10 md:-ml-11 scroll-smooth">
  ...
  <div className="text-sm lg:text-lg font-semibold -mt-3">
  Total Expenses: 
